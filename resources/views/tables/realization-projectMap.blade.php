@if ($project && $project->basicReference && $project->basicInfo && $project->basicReference->isNotEmpty() && $project->basicInfo->isNotEmpty())
                ...
                <table id="workGroup-table" class="">
                    ...
                    @foreach ($project->workGroup ?? [] as $item)
                        <tr>
                            ...
                            <td> Стратегическое управление проектом. <br> Выделение ресурсов и разрешения конфликтных
                                ситуаций. <br> Определение уровня прибыли стадий проекта.
                            </td>
                        </tr>
                        <tr>
                            <td>Руководитель проекта</td>
                            <td>{{ $item->projDirector }}</td>
                            <td> Формирование расчетной базы проекта/стадии проекта. <br> Ведение деловой переписки с
                                Заказчиком. <br> Инициация задач и контроль исполнения. <br> Оперативное управление
                                проектом. <br> Регулярный контроль бюджета, сроков реализации стадии проекта.<br>
                                Контроль рисков и инициация выработки корректирующих мероприятий. <br> Формирование
                                отгрузочных документов для закрытия проекта/стадии проекта. <br> Итоговая отчетность.
                            </td>
                        </tr>
                        <tr>
                            <td>Техлид</td>
                            <td>{{ $item->techlid }}</td>
                            <td> Выработка технических решений в рамках реализации проекта. <br> Техническое
                                сопровождение проекта. <br> Выполнение экспертной оценки. <br> Разработка РКД с
                                инициацией соответствующих документов/сущностей в ERP.
                            </td>
                        </tr>
                        <tr>
                            <td>Производство</td>
                            <td>{{ $item->production }}</td>
                            <td> Курирование этапа производства. <br> Контроль бюджета производимого оборудования. <br>
                                Контроль сроков производства. <br> Контроль ресурса производственного блока. <br> Доклад
                                о статусе производства. <br> Создание соответсвующих документов/сущностей в ERP. <br>
                                Инициация закупа.
                            </td>
                        </tr>
                        <tr>
                            <td>Снабжение</td>
                            <td>{{ $item->supply }}</td>
                            <td> Обеспечение проекта необходимыми ТМЦ. <br> Доклад о статусе закупа. <br> Создание
                                соответсвующих документов/сущностей в ERP.
                            </td>
                        </tr>
                        <tr>
                            <td>Логист</td>
    ...
    <div class="d-flex gap-3 mt-5">
        <a href="{{ route('update-realization', ['id' => $project->id, 'tab' => 'realization']) }}">
            <button class="btn btn-primary">Редактировать</button>
        </a>
    </div>

@elseif($project && $project->totals && $project->totals->isNotEmpty())
    <a href="{{ route('realization-create', $project->id) }}" class="btn btn-primary" target="_blank">Добавить
        реализацию</a>
@else
